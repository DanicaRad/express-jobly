...
describe("POST /user/:username/jobs/:jobId", function () {
  let job;
  ...
  test("works for admin", async function() {
    const resp = await request(app)
            .post(`/users/u3/jobs/${job.id}`)
            .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({applied: job.id});
  });
  test("works for self, non-admin", async function() {
    const resp = await request(app)
          .post(`/users/u2/jobs/${job.id}`)
          .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({applied: job.id});
  });
  test("bad request if dupe application", async function () {
    const resp = await request(app)
...
describe("GET /users/:username", function () {
  ...
  test("works: logged in non-admin user can get self", async function () {
    const resp = await request(app)
        .get(`/users/u2`)
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({
...
describe("PATCH /users/:username", () => {
  ...
  test("works: user can edit self", async function () {
    const resp = await request(app)
        .patch(`/users/u2`)
        .send({
          firstName: "New",
...
describe("DELETE /users/:username", function () {
  ...
  test("works: non-admin user can delete self", async function () {
    const resp = await request(app)
        .delete(`/users/u2`)
        .set("authorization", `Bearer ${u2Token}`);
